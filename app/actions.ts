...
${todos?.map((todo) => `- ${todo.id}: ${todo.text} (${todo.emoji})`).join("\n")}
    ...
    : ""
        ...
        - If the action is "add", the text, emoji, targetDate, and optional subtasks should be included.
        ...
        - If the action is "add_subtask", the parentTodoId and the subtask text should be included.
        
        For the add action, the text should be in the future tense. like "buy groceries", "make a post with @theo", "go for violin lesson"
        ${
          emoji
            ? `Change the emoji to a more appropriate based on the text. The current emoji is: ${emoji}`
            : ""
        }
        ...
        Subtask Generation:
        - If the user asks to break down a task or add steps to a todo, use the "add_subtask" action.
        - Identify the parent todo using its ID.
        - Generate a list of subtasks based on the user's request.
        - Example: "todo id: 'abc123', text: 'Plan vacation', user request: 'break down plan vacation', action: 'add_subtask', parentTodoId: 'abc123', subtasks: [{text: 'Research destinations'}, {text: 'Book flights'}, {text: 'Book hotel'}]"
        - Example: "todo id: 'def456', text: 'Write blog post', user request: 'add steps for writing blog post', action: 'add_subtask', parentTodoId: 'def456', subtasks: [{text: 'Outline post'}, {text: 'Write draft'}, {text: 'Edit and proofread'}]"
    `;
    ...
    schema: z.object({
      actions: z.array(
        z.object({
          ...
          todoId: z
            .string()
            .describe("The id of the todo item to act upon")
            .optional(),
          emoji: z.string().describe("The emoji of the todo item").optional(),
          targetDate: z
            .string()
            .describe("The target date for the todo item in YYYY-MM-DD format")
            .optional(),
          time: z
            .string()
            .describe("The time for the todo item in HH:mm format (24-hour)")
            .optional(),
          subtasks: z
            .array(z.object({ text: z.string() }))
            .describe(
              "An array of subtasks for the 'add' or 'add_subtask' action"
            )
            .optional(),
          parentTodoId: z
            .string()
            .describe("The ID of the parent todo for the 'add_subtask' action")
            .optional(),
          sortBy: z
            .enum(["newest", "oldest", "alphabetical", "completed"])
            .describe("The sort order")
            .optional(),
          status: z
            .enum(["complete", "incomplete"])
            .describe(
              "The status of the todo item. to be used for the mark action"
            )
            .optional(),
          listToClear: z
